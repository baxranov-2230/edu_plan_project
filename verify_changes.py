...
async def verify_user_flow():
    ...
    req = urllib.request.Request(f"{BASE_URL}/users/", data=data_json, headers=headers, method="POST")
    ...
    try:
        ...
        logger.error(f"Failed to create user. Status: {e.code} Body: {e.read().decode('utf-8')}")
        return
    ...
    if "student" not in created_roles or "teacher" not in created_roles:
        logger.error(f"Roles mismatch! Expected ['student', 'teacher'], got {created_roles}")
        return
    ...
    if user_data.get("passport_series") != passport:
        logger.error(f"Passport mismatch: {user_data.get('passport_series')} != {passport}")
        return
    ...
    status, updated_data = make_request("PATCH", f"{BASE_URL}/users/{user_id}", update_payload)
