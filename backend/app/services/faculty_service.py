...
class FacultyService:
    async def get_multi(self, db: AsyncSession, skip: int = 0, limit: int = 100) -> List[Faculty]:
    ...
    async def create(self, db: AsyncSession, faculty_in: FacultyCreate) -> Faculty:
        ...
        faculty = Faculty(
            ...
            description=faculty_in.description
        )
        db.add(faculty)
        await db.commit()
    ...
    async def update(self, db: AsyncSession, *, db_obj: Faculty, obj_in: FacultyUpdate) -> Faculty:
