...
class StreamService:
    ...
    async def get(self, db: AsyncSession, id: int) -> Optional[Stream]:
        result = await db.execute(select(Stream).options(selectinload(Stream.groups)).where(Stream.id == id))
    ...
    async def create(self, db: AsyncSession, obj_in: StreamCreate) -> Stream:
        ...
        if obj_in.group_ids:
            # Fetch groups
            groups_result = await db.execute(select(Group).where(Group.id.in_(obj_in.group_ids)))
    ...
    async def update(self, db: AsyncSession, *, db_obj: Stream, obj_in: StreamUpdate) -> Stream:
