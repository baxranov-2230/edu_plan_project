...
async def fix_data():
    ...
    async with SessionLocal() as db:
        ...
        for slug in all_permissions:
            result = await db.execute(text("SELECT id FROM permissions WHERE slug = :slug"), {"slug": slug})
            perm_row = result.first()
            if not perm_row:
                logger.info(f"Creating permission: {slug}")
                res = await db.execute(text("INSERT INTO permissions (slug, description) VALUES (:slug, :desc) RETURNING id"), {"slug": slug, "desc": slug})
                perm_id = res.scalar()
            else:
                perm_id = perm_row[0]
        ...
        for role_name in ["admin", "teacher", "student", "department_head"]:
            ...
            if not role_row:
                ...
                res = await db.execute(text("INSERT INTO roles (name, description) VALUES (:name, :desc) RETURNING id"), {"name": role_name, "desc": f"{role_name} role"})
                role_id = res.scalar()
            else:
                role_id = role_row[0]
            ...
            for slug in role_perms_slugs:
                ...
                if perm_id:
                    # Check link
                    link_res = await db.execute(text("SELECT 1 FROM role_permissions WHERE role_id=:rid AND permission_id=:pid"), {"rid": role_id, "pid": perm_id})
                    if not link_res.scalar():
                         await db.execute(text("INSERT INTO role_permissions (role_id, permission_id) VALUES (:rid, :pid)"), {"rid": role_id, "pid": perm_id})
        ...
        if user:
            user_id = user[0]
            # Update password
            update_stmt = text("UPDATE users SET hashed_password = :pwd WHERE email = :email")
        ...
        if user_id:
             # Find admin role id
             role_project = await db.execute(text("SELECT id FROM roles WHERE name = 'admin'"))
             role_id = role_project.scalar()
             
             # Check if link exists
             link_exists = await db.execute(text("SELECT 1 FROM user_roles WHERE user_id = :uid AND role_id = :rid"), {"uid": user_id, "rid": role_id})
